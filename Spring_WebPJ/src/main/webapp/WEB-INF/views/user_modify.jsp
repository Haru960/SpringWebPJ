...
<%
	...
	try{
		...
		response.sendRedirect("user_modify"); 
	}
%>
<!DOCTYPE html>
<html>
...
<meta charset="UTF-8">
<!-- css 파일 -->
<link href="${pageContext.request.contextPath}/resources/css/css_modify.css?v=3"
...
<body>
	...
	<script type="text/javascript" src="${pageContext.request.contextPath}/resources/js/bootstrap.js" charset="UTF-8"></script>
	<script type="text/javascript" src="${pageContext.request.contextPath}/resources/js/members.js" charset="UTF-8"></script>
	<script type="text/javascript" src="${pageContext.request.contextPath}/resources/js/myPage.js" charset="UTF-8"></script>
	<!-- 
	<script>
	...
	<div class="grid_container">
		...
		<div class="right_side">
			...
			<div class="mywrite" <% if(!ck.equals("mw")) out.print("hidden"); %>>
				<div class="search">
					<form class="table-from" action="user_modify" method="get">
					...
					<c:forEach items="${list}" var="dto">
						<tr class="trs">
							...
							<td class="border_td" id="td_title"><a
								href="bcontent_view?bId=${dto.bId}">${dto.bTitle}</a></td>
							...
							<td id="td_del"><a href="delete?bId=${dto.bId}">삭제</a></td>
						</tr>
					</c:forEach>
				</table>
				<div class="paging">
					<ul class="pagination">
						...
						<li><a
							href="user_modify?PN=<%= firstPage %>&searchType=<%=searchType%>&ck=mw"
							class="bt"> &lt;&lt; </a></li>
						...
						<li><a
							href="user_modify?PN=<%= Integer.parseInt(PN) - 1 %>&searchType=<%=searchType%>&ck=mw"
							class="bt"> &lt; </a></li>
						...
						<li><a
							href="user_modify?PN=<%= Integer.parseInt(PN) %>&searchType=<%=searchType%>&ck=mw"
							class="bt"> &lt; </a></li>
						...
						%>
						<li><a
							href="user_modify?PN=<%= i %>&searchType=<%=searchType%>&ck=mw"
							class="bt"><%= i %> </a></li>
						<%
						}
						%>
						<li class="active"><a
							href="user_modify?PN=<%= PN %>&searchType=<%=searchType%>&ck=mw"
							class="bt"><%= PN %> </a></li>
						<%
						...
						&nbsp;
						<li><a
							href="user_modify?PN=<%=Integer.parseInt(PN) + 1 %>&searchType=<%=searchType%>&ck=mw&ck=mw"
							class="bt"> &gt; </a></li>
						<% 
						if(targetPage + Integer.parseInt(PN) > startPage + 9){
						%>
						<li><a
							href="user_modify?PN=<%= startPage+10 %>&searchType=<%=searchType%>&ck=mw"
							class="bt"> &gt;&gt; </a></li>
						<%
						} else {
						%>
			...
			<div class="mywrite" <% if(!ck.equals("myq")) out.print("hidden"); %>>
					...
					<c:forEach items="${qlist}" var="dto">
						<tr class="trs">
							...
							<td id="td_del">
								...
								<c:if test="${dto.aId == 0}">
									<a href="qa_m_delete?qId=${dto.qId}">삭제</a>
								</c:if>     
							</td>
						</tr>
					</c:forEach>
				...
				<div class="paging">
					<ul class="pagination">
						...
						<li><a
							href="user_modify?PN=<%= firstPage %>&searchType=<%=searchType%>&ck=myq"
							class="bt"> &lt;&lt; </a></li>
						...
						<li><a
							href="user_modify?PN=<%= Integer.parseInt(PN) - 1 %>&searchType=<%=searchType%>&ck=myq"
							class="bt"> &lt; </a></li>
						...
						<li><a
							href="user_modify?PN=<%= Integer.parseInt(PN) %>&searchType=<%=searchType%>&ck=myq"
							class="bt"> &lt; </a></li>
						...
						%>
						<li><a
							href="user_modify?PN=<%= i %>&searchType=<%=searchType%>&ck=myq"
							class="bt"><%= i %> </a></li>
						<%
						}
						%>
						<li class="active"><a
							href="user_modify?PN=<%= PN %>&searchType=<%=searchType%>&ck=myq"
							class="bt"><%= PN %> </a></li>
						<%
						for(int i = Integer.parseInt(PN) + 1; i <= targetPage; i++){
							if(i < startPage + 10) {
						...
						&nbsp;
						<li><a
							href="user_modify?PN=<%=Integer.parseInt(PN) + 1 %>&searchType=<%=searchType%>&ck=mw&ck=myq"
							class="bt"> &gt; </a></li>
						<% 
						if(targetPage + Integer.parseInt(PN) > startPage + 9){
						%>
						<li><a
							href="user_modify?PN=<%= startPage+10 %>&searchType=<%=searchType%>&ck=myq"
							class="bt"> &gt;&gt; </a></li>
						<%
						} else {
						%>
