...
<meta charset="UTF-8">
<!-- css 파일 -->
<link href="${pageContext.request.contextPath}/resources/css/css_list.css?v=3"
...
<body>
	...
	<div class="grid_container">
		...
		<div class="search">
			<form class="table-from" action="list?PN=<%= 1 %>" method="get">
			...
			<table class="board">
				...
				<c:forEach items="${list}" var="dto">
					<tr class="trs">
						...
						<td class="border_td" id="td_title"><a href="bcontent_view?bId=${dto.bId}">${dto.bTitle}</a></td>
				...
				<tr id="tr_final">
					<td colspan="5"><a href="bwrite_view">글 작성</a></td>
				</tr>
			</table>
			</div>
			<div class="paging">
				<ul class="pagination">
					...
					%>
						...
						<li><a href="list?PN=<%= firstPage %>&search=<%=search%>&searchType=<%=searchType%>" class="bt"> &lt;&lt;  </a> </li>
						
					<%
						if(Integer.parseInt(PN) != 1) {
					%>
					
						<li><a href="list?PN=<%= Integer.parseInt(PN) - 1 %>&search=<%=search%>&searchType=<%=searchType%>" class="bt"> &lt;  </a> </li>
						
					<%
						} else {
					%>
					
						<li><a href="list?PN=<%= Integer.parseInt(PN) %>&search=<%=search%>&searchType=<%=searchType%>" class="bt"> &lt;  </a> </li>
					...
					%>
						<li><a href="list?PN=<%= i %>&search=<%=search%>&searchType=<%=searchType%>" class="bt"><%= i %> </a> </li>
					<%
						}
					%>
						<li class="active"><a href="list?PN=<%= PN %>&search=<%=search%>&searchType=<%=searchType%>" class="bt"><%= PN %> </a> </li>
					<%
					...
					%>
					<li><a href="list?PN=<%= startPage+10 %>&search=<%=search%>&searchType=<%=searchType%>" class="bt"> &gt;&gt; </a> </li>
					<%
					} else {
					%>
					<li><a href="list?PN=<%=PN%>&search=<%=search%>&searchType=<%=searchType%>" class="bt"> &gt;&gt; </a> </li>
					<%
					}
					%>
				</ul>
